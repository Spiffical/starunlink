...
class ConvLayer(nn.Module):
    ...
    def get_output_size(self, input_size):
        ...
        # print('ConvLayer input_size: ', input_size)
        # Transposed
        if self.transpose:
            assert (input_size > 1)
        ...
        # Conv
        # print('ConvLayer curr_size: ', curr_size)
        curr_size = curr_size - self.kernel_size + 1  # o = i + p - k + 1
        assert (curr_size > 0)
...
class UpsamplingBlock(nn.Module):
    ...
    def get_output_size(self, input_size):
        ...
        # print(f'UpSampling input size: {input_size}')
        curr_size = self.upconv.get_output_size(input_size)
        # print(f'UpSampling curr size: {curr_size}')

        # Upsampling convs
        for conv in self.pre_shortcut_convs:
            curr_size = conv.get_output_size(curr_size)
            # print(f'UpSampling pre_shortcut_convs curr size: {curr_size}')

        # Combine convolutions
        for conv in self.post_shortcut_convs:
            curr_size = conv.get_output_size(curr_size)
            # print(f'UpSampling post_shortcut_convs curr size: {curr_size}')
...
class Waveunet(nn.Module):
    def __init__(self, num_inputs, num_channels, num_outputs, instruments, kernel_size, target_output_size, conv_type,
                 res, separate=False, depth=1, strides=2):
        ...
        for instrument in model_list:
            ...
            for i in range(self.num_levels - 1):
                ...
                module.downsampling_blocks.append(
                    DownsamplingBlock(in_ch, num_channels[i], num_channels[i + 1], kernel_size, strides, depth,
                                      conv_type, res))

            for i in range(0, self.num_levels - 1):
                module.upsampling_blocks.append(
                    UpsamplingBlock(num_channels[-1 - i], num_channels[-2 - i], num_channels[-2 - i], kernel_size,
